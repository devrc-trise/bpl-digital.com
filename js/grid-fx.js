...
;(function(window) {
	...
	function GridFx(el, options) {
		...
		this.prevCtrl = this.previewEl.querySelector('button.action--previous');
	...
	GridFx.prototype._initEvents = function () {
		...
		// navigate previous expanded image
		this.prevCtrl.addEventListener('click', function() {
			self._prevItem();
		});

		// navigate next expanded image
	...
	GridFx.prototype._openItem = function(ev, item) {
		if ((this.isAnimating || this.isExpanded) && !this.isNavigating) return;
		...
		if (this.current == 0) {
			this.prevCtrl.style.display = 'none';
			this.nextCtrl.style.display = 'inline-block';
		} else if (this.current == this.items.length - 1) {
			this.prevCtrl.style.display = 'inline-block';
			this.nextCtrl.style.display = 'none';
		} else {
			this.prevCtrl.style.display = 'inline-block';
			this.nextCtrl.style.display = 'inline-block';
		}
	...
	GridFx.prototype._setClone = function(src, settings) {
		if( !src ) {
			...
			this.cloneImg.style.opacity = this.isNavigating ? 0 : 1;
	 ...
	 */
	GridFx.prototype._prevItem = function() {
		if (this.current == 0) return;
		var prevItem = this.items[this.current - 1];
		this._navigateItem(prevItem);
	};

	/**
	 * navigate to next item
	 */
	GridFx.prototype._nextItem = function() {
		if (this.current == this.items.length - 1) return;
		var nextItem = this.items[this.current + 1];
		this._navigateItem(nextItem);
	};

	GridFx.prototype._navigateItem = function(item) {
		var gridItem = this.items[this.current];
		classie.remove(gridItem, 'grid__item--current');
		this.isNavigating = true;
		this._openItem(null, item);
		this.isAnimating = false;
